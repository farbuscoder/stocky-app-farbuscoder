...
const Carbones = () => {
  ...
  const carbones = datos.filter((dato) => dato.Categoria === "Carbones");

  const filtrados = carbones.filter((carbon) => carbon.Marca === wordToFilter);
    ...
    <div className="display">
      ...
      <div className="content">
        <div className="grid-container">
          {(filtrados.length == 0 ? carbones : filtrados).map((carbon) => {
            return <Item key={carbon._id} dato={carbon} />;
          })}
        </div>
      </div>
    </div>
