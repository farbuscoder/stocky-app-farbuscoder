...
const Repuestos = () => {
  ...
  const repuestos = datos.filter((dato) => dato.Categoria === "Repuestos");

  const filtrados = repuestos.filter(
    (repuestos) => repuestos.Marca === wordToFilter
  );
    ...
    <div className="display">
      ...
      <div className="content">
        <div className="grid-container">
          {(filtrados.length == 0 ? repuestos : filtrados).map((repuesto) => {
            return <Item key={repuesto._id} dato={repuesto} />;
          })}
        </div>
      </div>
    </div>
