...
const Herramientas = () => {
  ...
  const herramientas = datos.filter(
    (dato) => dato.Categoria === "Herramientas"
  );

  const filtrados = herramientas.filter(
    (herramienta) => herramienta.Marca === wordToFilter
  );
    ...
    <div className="display">
      ...
      <div className="content">
        <div className="grid-container">
          {(filtrados.length == 0 ? herramientas : filtrados).map(
            (herramienta) => {
              return <Item key={herramienta._id} dato={herramienta} />;
            }
          )}
        </div>
      </div>
